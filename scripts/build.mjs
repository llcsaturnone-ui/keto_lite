...
<meta name="description" content="Питание и прогресс: КБЖУ, избранные продукты, замеры тела и графики по дням.">
...
const CACHE = ${JSON.stringify(`nutrition-v12-${path.basename(appFile.path, '.js')}`)};
const ASSETS = ${JSON.stringify(shellAssets)};
self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(ASSETS)).then(() => self.skipWaiting()));
});
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => (key.startsWith('nutrition-v12-') || key === 'kbju-trainer-v1') && key !== CACHE).map(key => caches.delete(key)))).then(() => self.clients.claim()));
});
